---
name: deploy-k8s
description: "Go 服务 Kubernetes 部署工作流 - 预检（测试/lint/工作区干净）→ 构建推送镜像 → helm template + kubeconform 校验清单 → helm upgrade --install → rollout 与健康验证 → 失败回滚；由 k8s-devops 子代理执行，block-dangerous Hook 守护生产操作。适用：新服务首次部署、版本升级、回滚到历史 revision、部署前 manifest 审查（资源限制/probe/PDB/securityContext）。不适用：编写 client-go 控制器或 Operator（用 k8s-go）、集群搭建与节点运维、非 Kubernetes 部署目标（VM/Serverless）。触发词：deploy, 部署, kubernetes, k8s, helm, rollout, rollback, 回滚, kubeconform, manifest, 上线, 发布"
argument-hint: "[deploy|rollback|review] [--context <ctx>] [--namespace <ns>] [--release <name>]"
---

# Kubernetes 部署工作流

从代码到 Kubernetes 集群的标准化部署流程。操作：$ARGUMENTS
...
| `golangci-lint` v2.8.0 | 预检 lint | `golangci-lint version` |
...
| 组件 | 类型 | 必需 |
|------|------|------|
| `k8s-devops` | Agent（`thoth:k8s-devops`） | 是 |
| `k8s-go` | Skill | 推荐 |
| `block-dangerous.sh` | Hook | 是（防误操作） |
| `session-context.sh` | Hook | 推荐 |
| `kubernetes` | MCP（go-backend-full 配置） | 推荐 |
...
1. 生产环境所有破坏性操作先 `--dry-run=client`，输出完整命令等用户确认后再执行
2. 镜像以 digest 记录，回滚以 helm revision 为准
3. 未通过预检（测试/lint/工作区干净）不进入构建
4. 清单校验失败不进入部署
5. 验证失败立即回滚，再排查

## Agent 调用方式

部署工作流由 `k8s-devops` 子代理主导执行，全流程通过单次 Agent 调用完成。`block-dangerous.sh` Hook 在后台自动守护，拦截生产环境误操作。

### 标准部署

用 Agent 工具调用 `thoth:k8s-devops` 子代理，prompt 只描述任务与约束：

```
Agent(
  subagent_type = "thoth:k8s-devops",
  prompt = """
  ## 任务
...
如果被拦截，子代理会收到 stderr 反馈并调整方案，无需人工干预。
...
- k8s-go Skill：client-go 控制器、Informer、Pod 操作
- https://helm.sh/docs/
