...
#
# 用法：
#   mock-claude.sh --team T1 --stance pro --round 1 --case <case-name> \
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --input)  shift 2 ;;
        --help)   echo "mock-claude.sh — see file header"; exit 0 ;;
        *)        shift ;;
    esac
done

[ -n "$case_name" ] || { echo "mock-claude: missing --case" >&2; exit 99; }

mocks_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")/scripts" && pwd 2>/dev/null || true)"
[ -d "$mocks_dir" ] || { echo "mock-claude: scripts dir not found: $mocks_dir" >&2; exit 99; }
...
echo "mock-claude: no fixture for case=$case_name team=$team stance=$stance round=$round" >&2
exit 99
