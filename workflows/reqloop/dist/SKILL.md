---
name: reqloop
description: 需求自验收闭环 — 以多 skill 编排完成"需求 → 代码 → 反讲 → 验收"。区别于普通 code review：先对齐需求再审代码；以代码为准（安全例外除外），但回写需求。支持 EARS 结构化反讲 + 判定三元组 + 决策链 + 负向影响分析。外部系统（需求管理 / 代码托管 / CI / 测试平台 / 缺陷系统）通过适配器接入，内置 lite 适配器只需 git。当用户要求"需求自验收"、"反讲需求"、"验收某个需求"、或提供需求 ID / PR 号希望做闭环验收时调用。支持 `--adapter <name>`，`--lite` 等价于 `--adapter lite`。
version: 0.2.1
---
